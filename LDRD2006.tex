...
\begin{figure}
  \centering
  \sticky{A nice ball and stick drawing of a simple graph.}
...
\begin{figure}
  \centering
  \sticky{A larger graph with balls as points and sticks as lines.}
...
built from the Enron email database\lcite{Klimt04} containing
\sticky{Vertices} vertices and \sticky{Edges} edges.  This images is
completely saturated with edge data.  We have reached the limit of our
display and the amount of information we get from edges is reduced to zero.
...
\begin{figure}
  \centering
  \fbox{Crossing perpendicular bisectors} \fbox{Crossing near parallel}
  \fbox{Close to parallel, non-intersecting lines} \fbox{Parallel Lines}
...
section will describe the outline for determining all of the edge
similarities.
...
\begin{figure}
  \centering
  \sticky{Image of a line sweeping a graph.}
  \caption{A sweep line on a graph.}
  \label{fig:SweepLine}
\end{figure}
...
\begin{figure}
  \centering
  \sticky{Make a nice graphic.  Use Peak Identification, Contributor
    Identification, and Label Extraction for the process blocks rather than
    VTK classes.}
  \begin{verbatim}
    [landscape height image]
               |
               v
vtkPImagePeakIdentificationFilter
               |
          [peak points]    [parallel graph, ghost points removed]
               |                  |      |
               v                  v      |
      vtkFindContributors <--- [kd-tree] |
               |                         |
  [peak points with contributor sets]    |
               |                         |
               |   +---------------------+
               v   v
      vtkPExtractLabels
