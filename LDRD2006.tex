...
\keyterm[scientific~visualization]{scientific visualization}.  Scientific
...
embedded in \keyterm{VTK}, the Visualization Toolkit.  VTK provides a
...
\keyterm{ParaView}, which provides both a client/server architecture for
...
\keyterm[information~visualization]{information visualization}, which deals
with abstract data types that have no necessary connection to physical
space.  ParaView, designed as a scientific visualization tool, is not
equipped to handle this type of visualization.  However, the distinction
...
\keyterm[graph~layout!force~directed]{force directed}, which places
...
\keyterm[analytical~reasoning~process]{analytical reasoning
...
in \keyterm[data~parallelism]{data parallelism} mode.  In this mode, the
